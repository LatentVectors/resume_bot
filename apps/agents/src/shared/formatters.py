...
class ExperienceProtocol(Protocol):
    """Protocol for experience objects."""

    id: int
    company_name: str
    job_title: str
    start_date: Any  # datetime-like with strftime


class AchievementProtocol(Protocol):
    """Protocol for achievement objects."""
...
def format_experience_with_achievements(
    experience: ExperienceProtocol,
    achievements: list[AchievementProtocol],
) -> str:
...
def format_all_experiences(
    experiences: list[ExperienceProtocol],
    achievements_by_exp: dict[int, list[AchievementProtocol]] | None = None,
) -> str:
