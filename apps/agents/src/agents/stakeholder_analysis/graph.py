"""Stakeholder analysis agent graph.

Analyzes hiring stakeholders based on job description and user experience,
identifying key stakeholders, their priorities, and how they may perceive
...
class InputState(BaseModel):
    """Input state for stakeholder analysis agent."""
...
class OutputState(BaseModel):
    """Output state for stakeholder analysis agent."""

    analysis: str = Field(default="", description="Markdown formatted stakeholder analysis")


class InternalState(InputState, OutputState):
    """Internal state combining input and output for stakeholder analysis agent."""
...
class Node(StrEnum):
    """Node names for the stakeholder analysis graph."""
...
_prompt = load_prompt(PromptName.STAKEHOLDER_ANALYSIS)
...
def analyze(state: InternalState) -> PartialInternalState:
    """Analyze hiring stakeholders and their perceptions of the candidate.
    ...
    try:
        ...
        if not state.work_experience or not state.work_experience.strip():
            logger.warning("Stakeholder analysis called with empty work experience")
            return {"analysis": ""}
        ...
        if exc.response.status_code == 429:
            logger.error(f"Rate limit exceeded during stakeholder analysis: {exc}")
            return {"analysis": ""}
        logger.exception(f"HTTP error during stakeholder analysis: {exc}")
        return {"analysis": ""}
    except Exception as exc:
        logger.exception(
            f"Stakeholder analysis failed. Job description length: {len(state.job_description) if state.job_description else 0}, "
