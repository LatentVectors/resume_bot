...
def generate_template_html(
    ...
    *,
    ...
    """
    Generate or edit a resume HTML template via LLM.
    ...
    Args:
        ...
        image: Optional image as raw bytes (PNG assumed) or URL/data URL string.

    Returns:
        Clean raw HTML string for the resume template (no markdown fences).
    """
    # Compose the user content parts (text + optional image)
    if current_html:
        text_directive = (
            "Revise the provided HTML template per the user's instructions. "
        ...
        text_directive = (
            "Generate a complete ATS-safe resume template as raw HTML. "
            "Return ONLY the complete raw HTML with no explanations, no markdown, and no code fences.\n\n"
            "User instructions:\n" + (user_text or "")
        )
    ...
    system_text = resume_template_prompt
