...
class ResumeService:
    ...
    def render_preview(job_id: int, resume_data: ResumeData, template_name: str) -> bytes:
        ...
        Note: Caching is removed for API version. Frontend should handle caching.
        """
        if not isinstance(job_id, int) or job_id <= 0:
            raise ValueError("Invalid job_id")
        if not template_name or not template_name.strip():
        ...
        try:
            # Render PDF directly without caching (caching is frontend concern)
            return PDFService.render_preview_pdf_bytes(resume_data, template_name)
        except Exception as e:  # noqa: BLE001
            logger.exception(e)
            raise
    ...
    def render_canonical_pdf_bytes(job_id: int) -> bytes:
        ...
        try:
            ...
            return PDFService.render_resume_pdf_bytes(data, template)
        except Exception as e:  # noqa: BLE001
            logger.exception(e)
            raise
