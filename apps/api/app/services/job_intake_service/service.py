...
from app.services.job_intake_service.workflows import extract_experience_updates, run_resume_chat
...
class JobIntakeService:
    ...
    @staticmethod
    def extract_experience_proposals(session_id: int, job_id: int) -> list[ExperienceProposal]:
        ...
        **DEVELOPMENT MODE**: Returns in-memory proposal objects only. Does NOT persist to database.
        ...
        Args:
            session_id: Intake session ID.
            job_id: Job ID (used to get user_id and experiences).
        ...
        try:
            ...
            experiences = ExperienceService.list_user_experiences(job.user_id)
            if not experiences:
                logger.info("No experiences found for user %s", job.user_id)
                return []
